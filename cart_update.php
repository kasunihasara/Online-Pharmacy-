...
window.onclick = function(e) {
  ...
  var myDropdown2 = document.getElementById("myDropdown2");
   ...
   <?php
      ...
      $select = mysqli_query($con, "SELECT * FROM products WHERE pid = '$ccid'");
      while($row = mysqli_fetch_assoc($select)){
        $_SESSION['product_value'] = $row['pid'];
     ?>
  <form  action="cart_update2.php" method="post">
...
<input type="number" id="quantity" name="update" min="1" max="" value="1" style="margin-left: 840px; margin-top: -40px; width:50px"></a>
  
  </div>
  <div style="margin-top: 30px; ">
<button  style= "margin-left: 740px; width: 120px; height: 38px; border-radius: 10px; background-color: rgba(118, 179, 199, 1); border-color: rgba(118, 179, 199, 1); font-size: 17px; ">Update</button>
  </div>
  </form>  
